...
def check_makefile() -> bool:
    ...
    if missing:
        print(
            f"[-] Error: Makefile is missing required variables: {', '.join(missing)}"
        )
        return False
...
def check_app_json() -> bool:
    ...
    if missing:
        print(
            f"[-] Error: app.json is missing required properties: {', '.join(missing)}"
        )
        return False
    ...
    try:
        ...
        if tag_name in tags:
            ...
            if os.getenv("ALLOW_RE_RELEASE") == "true":
                print(
                    f"[!] Warning: Git tag {tag_name} already exists but ALLOW_RE_RELEASE is true. Proceeding."
                )
                return True
            print(
                f"[-] Error: Git tag {tag_name} already exists. Increment version or enable re-release."
            )
            return False
