...
def main():
    ...
    print(
        "  - Place the `.nro` file in the `/switch/pwa2nx/` directory on your SD card."
    )
    print(
        "  - Launch it using the Homebrew Launcher (preferably via Title Takeover / holding `R` while launching a game to avoid memory limits).\n"
    )
    print("* **NSP File (Home Screen Channel):**")
    print(
        "  - Install the `.nsp` file using any Switch title installer (such as Goldleaf, Awoo, or Tinfoil)."
    )
    print(
        "  - **Important:** The NSP is a forwarder. It requires the corresponding `.nro` file to be present in `/switch/pwa2nx/[PWA_NAME].nro` on your SD card to function.\n"
    )
    ...
    if nro_files:
        ...
        if has_nsp:
            ...
            for nro_path in nro_files:
                ...
                nro_url = (
                    f"https://github.com/{repo}/releases/download/{tag}/{nro_name}"
                )
                ...
                if os.path.exists(nsp_path):
                    nsp_url = (
                        f"https://github.com/{repo}/releases/download/{tag}/{nsp_name}"
                    )
                ...
                print(
                    f"| **{pwa_name}** | [{nro_name}]({nro_url}) | {nsp_cell} | {badge_cell} |"
                )
        else:
            ...
            for filepath in nro_files:
                ...
                print(
                    f"| **{pwa_name}** | [{filename}]({download_url}) | ![{pwa_name} Downloads]({badge_url}) |"
                )
        print("\n")
