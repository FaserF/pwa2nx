...
def main():
    ...
    subprocess.run(["npdmtool", "nsp_build/npdm.json", "nsp_build/exefs/main.npdm"], check=True)
    
    # 3. Strip ELF to exefs/main
    subprocess.run(["aarch64-none-elf-strip", "-o", "nsp_build/exefs/main", "pwa2nx.elf"], check=True)
    
    # 4. Copy NACP file
    if os.path.exists("pwa2nx.nacp"):
        subprocess.run(["cp", "pwa2nx.nacp", "nsp_build/control/control.nacp"], check=True)
    else:
        # Fallback build nacp
        subprocess.run(["nacptool", "--create", app_name, "FaserF", "1.0.0", "nsp_build/control/control.nacp"], check=True)
        
    # 5. Convert icon to JPEG and place as icon_AmericanEnglish.dat
    if os.path.exists("source/icon.jpg"):
        subprocess.run(["cp", "source/icon.jpg", "nsp_build/control/icon_AmericanEnglish.dat"], check=True)
        
    # 6. Check for keys
    has_keys = os.path.exists(os.path.expanduser("~/.switch/prod.keys")) or os.path.exists("prod.keys") or os.path.exists("keys.dat")
    
    hacbrewpack_cmd = ["hacbrewpack", "--nspdir", ".", "--nopatch"]
    if not has_keys:
        print("Warning: Keys not found. Generating plaintext (unencrypted) NSP fallback...")
