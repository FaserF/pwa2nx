...
def parse_args():
    ...
    parser.add_argument("--output-icon", default="source/icon.png", help="Output path for Switch icon")
    parser.add_argument("--output-hdr", default="source/config.h", help="Output path for C config header")
...
def download_and_resize_icon(icon_url, dest_path):
    try:
...
def generate_default_icon(dest_path):
...
def main():
    ...
    if manifest_url:
        try:
            ...
            if mr.status_code == 200:
                ...
                app_name = manifest_data.get('short_name', manifest_data.get('name', app_name))
    ...
    if 'icons' in manifest_data and manifest_data['icons']:
        ...
        best_icon = sorted(icons, key=lambda x: int(x.get('sizes', '0').split('x')[0]) if 'x' in x.get('sizes', '') else 0, reverse=True)[0]
    ...
    if not found_icon:
        if os.path.exists(args.output_icon):
            print(f"No custom PWA icon fetched. Preserving existing fallback icon at {args.output_icon}")
        else:
            generate_default_icon(args.output_icon)

    # Extract GITHUB_REPO from app.json
    import json
    repo_owner_name = "fabian-seitz/pwa2nx"
    try:
